...
function diffusion_2D_perf(do_check)
    ...
    while err >= ϵtol && iter < maxiter
        ...
        if do_check && (iter % ncheck == 0)
            ...
            frame(anim, p3)
            # display(p3)
            png(p3, "images/pde/par/diffusion_2D_perf.png")
        end
    ...
    # Save to gif file
    if do_check
        path = "images/pde/par/diffusion_2D_perf.gif"
        gif(anim, path, fps=FPS)
        println("Save gif result to $path")
    end
end
println("Complete the simulation of diffusion_2D_perf ", diffusion_2D_perf(true))
